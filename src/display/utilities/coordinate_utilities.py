...
def equirectangular_distance(start: Tuple[float, float], finish: Tuple[float, float]) -> float:
    """
...
def get_centre_of_line_lat_lon(start: Tuple[float, float], finish: Tuple[float, float]) -> Tuple[float, float]:
...
#####
# Taken from https://stackoverflow.com/questions/32771458/distance-from-lat-lng-point-to-minor-arc-segment
...
def cross_track_distance(lat1, lon1, lat2, lon2, lat, lon) -> float:
    """
    The cross track distance in metres from (lat,lon) to the line between (lat1,lon2) - (lat2,lon2)
...
def create_rounded_corridor_corner(
    ...
    right_end = calculate_fractional_distance_point_lat_lon(
        bisecting_line[0],
        bisecting_line[1],
        0.5 + (corridor_width_metres / 2) / calculate_distance_lat_lon(*bisecting_line),
    )
    left_end = calculate_fractional_distance_point_lat_lon(
        bisecting_line[1],
        bisecting_line[0],
        0.5 + (corridor_width_metres / 2) / calculate_distance_lat_lon(*bisecting_line),
    )
    ...
    corrected_gate_line_perimeter = calculate_fractional_distance_point_lat_lon(
        circle_centre,
        circle_perimeter,
        corridor_width_metres / calculate_distance_lat_lon(circle_centre, circle_perimeter),
    )
    ...
    starting_point = norm_v(rotate_vector_angle(normalised_gate[0], normalised_gate[1], initial_offset))
    outer_edge = []
    inner_edge = []
