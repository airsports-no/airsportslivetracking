...
def get_destination_point(p: dict, distance, bearing):
    ...
    return {"lat": to_deg(lat2), "lng": to_deg(lon2)}
...
def generate_corridor_polygon(route_points: list[Waypoint], round_edges: bool = True) -> tuple[list[dict], list[dict]]:
    ...
    for i, p in enumerate(route_points):
        if i == 0:
            path_points.append(
                {"lat": p.latitude, "lng": p.longitude, "width": p.width * 1852, "is_waypoint": True}
            )  # Convert nm to m
            continue
...
def validate_corridor(route_points: list[Waypoint]) -> list[str]:
    ...
    for i in range(len(route_points)):
        p: Waypoint = route_points[i]

        if p.on_curved_segment:
            continue
