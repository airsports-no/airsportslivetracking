            ...
            if backtracking:
                if self.tracking_state == self.TRACKING:
                    ...
                    is_grace_time_after_steep_turn = last_gate.infinite_passing_time is not None and last_gate.is_steep_turn and (
                        ...
                        last_gate.type)
                    is_grace_distance_before_turn = next_gate.get_distance_to_gate_line(last_position.latitude,
                                                                                        last_position.longitude) / 1852 < self.scorecard.get_backtracking_before_gate_grace_period_nm_for_gate_type(
                        next_gate.type) if next_gate else False
                    is_grace_distance_after_turn = last_gate.get_distance_to_gate_line(last_position.latitude,
