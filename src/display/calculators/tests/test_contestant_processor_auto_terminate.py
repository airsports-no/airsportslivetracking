...
class TestContestantProcessorAutoTerminate(TestCase):
    ...
    def test_proactive_termination_on_low_speed(
        self,
        mock_slack,
        mock_calc_factory,
        mock_redis_queue,
        ...
        with patch("threading.Thread"):
            ...
            for i in range(65):
                ...
                for position in all_positions:
                    ...
                    if (
                        processor.live_processing
                        and processor.orchestrator.has_any_gate_passed
                        and not processor.track_terminated
                    ):
    ...
    @patch("display.calculators.contestant_processor.calculator_factory")
    @patch("display.calculators.contestant_processor.post_slack_competition_message")
    def test_no_termination_if_no_gates_passed(
        self,
        mock_slack,
        mock_calc_factory,
        mock_redis_queue,
    ...
    @patch("display.calculators.contestant_processor.calculator_factory")
    @patch("display.calculators.contestant_processor.post_slack_competition_message")
    def test_reset_counter_on_high_speed(
        self,
        mock_slack,
        mock_calc_factory,
        mock_redis_queue,
