...
def add_contest_teams_to_navigation_task(request, pk):
    ...
    """
    TIME_LOCK_MINUTES = 15
    navigation_task = get_object_or_404(NavigationTask, pk=pk)
    contest_teams = []
    now = datetime.datetime.now(datetime.timezone.utc)
    for contest_team in navigation_task.contest.contestteam_set.all():
        try:
            contestant = navigation_task.contestant_set.get(team=contest_team.team)
            if contestant.takeoff_time - datetime.timedelta(minutes=TIME_LOCK_MINUTES) < now:
    ...
    for contest_team, speed, wind_speed, wind_direction, minutes_to_starting_point, minutes_to_landing in contest_teams:
        ...
        duration = timedelta(minutes=minutes_to_starting_point + minutes_to_landing) + gate_times[-1][1]
        team_data.append(
    ...
    for team_definition in team_definitions:
        ...
        try:
            ...
            # TODO: could make track of start time part of the optimisation
            contestant.tracker_start_time = team_definition.start_time - timedelta(minutes=0)
            ...
            contestant = Contestant.objects.create(takeoff_time=team_definition.start_time,
                                                   ...
                                                   tracker_start_time=team_definition.start_time - timedelta(minutes=0),
