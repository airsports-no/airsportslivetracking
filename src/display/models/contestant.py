...
"""
    ...
    def clean(self):
        ...
        if self.finished_by_time - self.tracker_start_time > datetime.timedelta(hours=24):
            raise ValidationError(
                f"The maximum tracking time for {self} (from tracker start time {self.tracker_start_time} to finished by time {self.finished_by_time}) is 24 hours (currently {self.finished_by_time - self.tracker_start_time}). Either start tracking later or finish earlier to solve this."
            )
    ...
    @property
    def gate_times_actual(self) -> dict:
        """
        Returns a dictionary of actual gate passing times.
        """
        return {item.gate: item.time for item in self.actualgatetime_set.all()}
