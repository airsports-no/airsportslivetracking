...
class TestGetContestantForDevice(TransactionTestCase):
    ...
    def test_get_tracking_device(self):
        contestant, simulator = Contestant.get_contestant_for_device_at_time(
            TRACKER_NAME, datetime.datetime(2020, 1, 1, 10, tzinfo=datetime.timezone.utc)
        )
        self.assertEqual(self.contestant_tracking_device, contestant)

    def test_get_no_pilot_device(self):
        contestant, simulator = Contestant.get_contestant_for_device_at_time(
            self.team.crew.member1.app_tracking_id, datetime.datetime(2020, 1, 1, 10, tzinfo=datetime.timezone.utc)
        )
    ...
    def test_get_tracking_pilot(self):
        contestant, simulator = Contestant.get_contestant_for_device_at_time(
            self.team.crew.member1.app_tracking_id, datetime.datetime(2020, 1, 1, 12, 3, tzinfo=datetime.timezone.utc)
        )
        self.assertEqual(self.contestant_pilot_device, contestant)

    def test_get_pilot_no_tracking_device(self):
        contestant, simulator = Contestant.get_contestant_for_device_at_time(
            TRACKER_NAME, datetime.datetime(2020, 1, 1, 12, 3, tzinfo=datetime.timezone.utc)
        )
        self.assertEqual(None, contestant)
    ...
    def test_get_copilot_no_tracking_device(self):
        contestant, simulator = Contestant.get_contestant_for_device_at_time(
            TRACKER_NAME, datetime.datetime(2020, 1, 1, 14, 3, tzinfo=datetime.timezone.utc)
        )
        self.assertEqual(None, contestant)

    def test_get_copilot_no_pilot_device(self):
        contestant, simulator = Contestant.get_contestant_for_device_at_time(
            self.team.crew.member1.app_tracking_id, datetime.datetime(2020, 1, 1, 14, 3, tzinfo=datetime.timezone.utc)
        )
        self.assertEqual(None, contestant)
