...
class TestContractNavigationContestantTeamIdApi(TestCase):
    @patch("display.models.contestant.get_traccar_instance", return_value=TraccarMock)
    @patch("display.signals.get_traccar_instance", return_value=TraccarMock)
    def setUp(self, *_args):
        create_scorecards()
        self.user = get_user_model().objects.create(email="contestant-teamid-api@example.com")
        Person.objects.create(first_name="Contestant", last_name="TeamIdApi", email=self.user.email)
        self.scorecard = Scorecard.get_originals().get(shortcut_name="FAI Precision")
        with open("display/tests/NM.csv", "r") as file:
            editable_route, _ = EditableRoute.create_from_csv("Contestant TeamId API route", file.readlines()[1:])
            self.route = editable_route.create_precision_route(True, self.scorecard)

        self.contest = Contest.objects.create(
            name="Contestant TeamId API Contest",
        ...
        self.navigation_task = NavigationTask.create(
            name="Contestant TeamId API Task",
        ...
        self.editable_route = EditableRoute.objects.create(
            name="Contestant TeamId API primitives",
            route={
        ...
        team = Team.objects.create(
            crew=Crew.objects.create(member1=Person.objects.create(first_name="Pilot", last_name="One", email="pilot-teamid@example.com")),
            aeroplane=Aeroplane.objects.create(registration="LN-CTID"),
        )
    ...
    def test_contestant_teamid_api_create_persists_contract_declaration_payload(self, _mock_guard):
