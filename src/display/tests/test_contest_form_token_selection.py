...
class TestContestFormTokenSelection(TestCase):
    def setUp(self):
        ...
        self.grant = UserTokenGrant.objects.create(user=self.user, token_type=self.large, quantity_total=1)

    def test_contest_form_can_limit_token_choices_to_supplied_queryset(self):
        form = ContestForm(token_grant_queryset=UserTokenGrant.objects.filter(pk=self.grant.pk))

        self.assertEqual([self.grant.pk], list(form.fields["initial_token_grant"].queryset.values_list("pk", flat=True)))
