...
class Traccar:
    ...
    def get_positions_for_device_id(
        ...
        if response.status_code == 200:
            ...
            logger.error(
                f"Failed fetching positions for device {device_id}, {response.text}"
            )
            return []
    ...
    def get_device_ids_for_contestant(self, contestant: "Contestant") -> List[int]:
        devices = []
        for name in (
            contestant.get_tracker_ids() + contestant.get_simulator_tracker_ids()
        ):
            try:
    ...
    def create_group(self, group_name) -> Dict:
        response = self.session.post(
            self.base + "/api/groups", json={"name": group_name}
        )
        if response.status_code == 200:
            return response.json()
    ...
    def get_device(self, identifier) -> Optional[Dict]:
        response = self.session.get(
            self.base + "/api/devices/?uniqueId={}".format(identifier)
        )
        if response.status_code == 200:
    ...
    def update_device_name(self, device_name: str, identifier: str) -> bool:
        ...
        if existing_device is None:
            logger.warning(
                "Failed fetching assumed to be existing device {}".format(identifier)
            )
            return False
        ...
        if response.status_code != 200:
            logger.error(
                f"Failed updating device name because of: {response.status_code} {response.text}"
            )
            return False
