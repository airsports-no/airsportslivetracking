...
**External traffic feed**: `position_processor.py`'s `live_position_transmitter_process` (`src/live_position_transmitter.py`) also pushes every non-simulator-tracked position (see `Person.simulator_tracking_id`'s help text) to `ws/traffic/airsports/` (`display.consumers.AirsportsPositionsConsumer`) — this is the live feed ASLT provides *outward* for partners (SafeSky) to consume, not something ASLT consumes. `display.consumers.GlobalConsumer` (`ws/tracks/global/`) fed a frontend global-traffic map that no longer exists and is now dead weight only kept for `transmit_global_position_data`'s own live positions.
...
**Live calculators**: per-contestant scoring calculators run as Celery tasks (`display.tasks.run_live_contestant_calculator`, one prefork worker process per contestant) on a dedicated `live_calculator` queue/worker pool (`helm/templates/deployment_live_calculator.yaml`), dispatched from `position_processor_process.py`'s `add_positions_to_calculator`. The pool is scaled by `src/calculator_pool_scaler.py`, a loop running inside `tracker-processor` that reads `Contestant.tracker_start_time` directly and patches the Deployment's replica count via the Kubernetes API — not KEDA/HPA, so it can pre-warm ahead of a known contest start rather than only reacting after a task is already queued. That periodic reconcile only runs every `CALCULATOR_POOL_POLL_SECONDS` (default 60s), so `add_positions_to_calculator` also calls `wake_pool_if_cold()` synchronously at the moment of a contestant's first dispatch — if the pool is at zero replicas (e.g. a contestant created with an immediate `tracker_start_time`, too late for the prewarm lookahead to have already scaled up), this bumps it straight to one instead of leaving the task queued for up to a minute; exact sizing beyond that still belongs to the periodic reconcile. `Orchestrator.track` (`display/calculators/orchestrator.py`) is a bounded `deque`, not an unbounded list, so per-contestant memory stays flat regardless of how long a calculator runs — relevant when working on scaling or calculator orchestration.
