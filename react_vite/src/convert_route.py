...
def convert_legacy_json(data):
    features = []
    
    route_layers = data.get('route', [])
    ...
    if track_layer:
        ...
        for i, point in enumerate(track_points):
            ...
            raw_width = point.get('gateWidth', 1000)
            width = raw_width * 1000 if raw_width < 10 else raw_width
            
            wp_feature = {
                "type": "Feature",
                "properties": {
                    "id": str(uuid.uuid4()),
                    "name": point.get('name', f"WP {i}"),
