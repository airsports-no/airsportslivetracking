...
import { getGateValue, getTrackValue } from "../../utils/scorecardUtils";
import type { Scorecard, RouteData } from "../../types";
...
const AboutAirsportChallenge: React.FC<Props> = ({ scorecard, route }) => {
    const gateWidths = route.waypoints.map((waypoint) => {
        return waypoint.width
    })
    return <div>
        <h2 className="text-2xl font-bold mb-4">AirSport Challenge</h2>
        <p>
            AirSport Challenge is a flying competition where the user is tasked with crossing a set of known and secret gates at specific
        ...
        <h3 className="text-xl font-bold my-2">Gate rules</h3>
        <p>
        ...
        <h3 className="text-xl font-bold my-2">Track rules</h3>
...
export default AboutAirsportChallenge;
