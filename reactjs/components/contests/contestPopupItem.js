    ...
    render() {
        ...
        return <div className={""} key={"contest" + this.props.contest.id}>
            ...
            <div className={""}>
                ...
                <span style={{"paddingTop": "0.5em", fontSize:"14px"}} className={"badge badge-dark badge-pill"}>{this.props.contest.contest_team_count} </span> <span style={{fontSize: "18px"}}>{new Date(this.props.contest.finish_time) > new Date() && document.configuration.authenticatedUser ?
                this.props.link?<Link to={"/participation/" + this.props.contest.id + "/register/"}>Register</Link>:<a href={"/participation/" + this.props.contest.id + "/register/"}>Register</a> : null}</span>
