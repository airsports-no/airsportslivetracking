...
class ConnectedContestantTrack extends Component {
    ...
    createAirplaneIcon(colour, opacity) {
        const size = 50;
        ...
        const solidPath = '<path style="opacity:' + opacity + ';fill:' + colour + ';stroke-width:0.8742;" d="' + little + '"/>'
        const outlinePath = '<path style="opacity:' + opacity + ';fill:black;stroke-width:0.93;" d="' + big + '"/>'
        return L.divIcon({
    ...
    createAirplaneTextIcon(colour, opacity) {
        const size = 20;
        const style = {
            color: colour,
            fontSize: size + 'px'
        ...
        return L.divIcon({
            html: '<div style="opacity:' + opacity + ';color: ' + colour + '; font-size: ' + size + 'px">' + contestantShortForm(this.props.contestant) + '</div>',
            iconAnchor: [100, -22],
            iconSize: [200, size],
    ...
    maybeUpdateAgeAndColour() {
        if (this.props.currentTime && this.props.contestantData && this.props.contestantData.contestant_track.current_state !== "Finished") {
            ...
            if (diff.as("seconds") > 15) {
                ...
                if (this.dotText) {
                    this.dotText.setTooltipContent(contestantLongForm(this.contestant) + "<br/>" + "Missing data: " + this.getAgeString(diff))
                    this.dot.setTooltipContent(contestantLongForm(this.contestant) + "<br/>" + "Missing data: " + this.getAgeString(diff))
                }
            } else if (this.missingData) {
                // Undim the contestant
                this.undim()
                this.missingData = false
            }
        }
    ...
    updateStyle() {
        ...
        this.dot.setIcon(this.createAirplaneIcon(this.props.colour, 1))
        this.dotText.setIcon(this.createAirplaneTextIcon(this.props.colour, 1))
    }
    ...
    styleContestant(style) {
        if (this.dot) {
            ...
            this.dot.setIcon(this.createAirplaneIcon(style.color, style.opacity))
            this.dotText.setIcon(this.createAirplaneTextIcon(style.color, style.opacity))
        }
    }
    ...
    createLiveEntities(positions) {
        ...
        this.dot = L.marker(newest_position, {icon: this.createAirplaneIcon(this.props.colour, 1)}).bindTooltip(contestantLongForm(this.contestant), {
            permanent: false
        }).on('click', (e) =>
            this.handleContestantLinkClick(e, this.contestant.id)
        ).on('mouseover', (e) => {
        ...
        this.dotText = L.marker(newest_position, {icon: this.createAirplaneTextIcon(this.props.colour, 1)}).bindTooltip(contestantLongForm(this.contestant), {
            permanent: false
        }).on('click', (e) =>
            this.handleContestantLinkClick(e, this.contestant.id)
        ).on('mouseover', (e) => {
